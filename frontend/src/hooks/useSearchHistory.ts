...
export const useSearchHistory = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<SearchHistoryItem[]> => {
      const { data } = await apiClient.get<SearchHistoryResponse>('/api/v2/search/history');
  ...
  const addToHistoryMutation = useMutation({
    mutationFn: async (keywords: string[]) => {
      await apiClient.post('/api/v2/search/history', { keywords });
    },
    onSuccess: () => {
      // 成功時に検索履歴を再取得
  ...
  const deleteFromHistoryMutation = useMutation({
    mutationFn: async (historyId: string) => {
      await apiClient.delete(`/api/v2/search/history/${historyId}`);
    },
    onSuccess: () => {
      // 成功時に検索履歴を再取得
      queryClient.invalidateQueries({ queryKey: ['searchHistory'] });
  ...
  const clearHistoryMutation = useMutation({
    mutationFn: async () => {
      await apiClient.delete('/api/v2/search/history');
    },
    onSuccess: () => {
      // 成功時に検索履歴を再取得
      queryClient.invalidateQueries({ queryKey: ['searchHistory'] });

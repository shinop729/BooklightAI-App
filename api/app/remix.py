...
class RemixService:
    ...
    async def generate_remix_essay(self, highlights: List[Dict[str, Any]], theme: str, title: str) -> str:
        ...
        # APIに送信するプロンプト
        prompt = f"""
        あなたは、様々な書籍から得たインスピレーションをもとに、独自の視点で魅力的なエッセイを書くエッセイストです。
        以下のハイライトはあなたの創作の素材であり、これらから得た洞察やアイデアをもとに、あなた自身の言葉でエッセイを書いてください。
        タイトル: {title}
        テーマ: {theme}
        ...
        【エッセイ作成の指針】
        1. 直接的な引用や「〜によれば」「〜では」といった引用スタイルは避け、あなた自身の言葉で語ってください
        2. ハイライトからインスピレーションは得つつも、あなた独自の視点で統合し、一貫性のある文章にしてください
        3. 書籍のタイトルや著者名を本文中で繰り返し言及するのではなく、内容やアイデアそのものに焦点を当ててください
        4. 起承転結の流れを持たせ、読み手を引き込む文体で書いてください
        5. 読者に新たな視点や気づきを与える内容を心がけてください
        6. 一人のエッセイストとしての一貫した文体を保ってください
